...
function toMistralRequest(model, input) {
  ...
  const messages = input.messages.map((msg) => {
    ...
    if (toolRequest == null ? void 0 : toolRequest.toolRequest) {
    ...
    if (toolResponse == null ? void 0 : toolResponse.toolResponse) {
      return {
        role: "tool",
  ...
  const request = __spreadValues(__spreadValues(__spreadValues({
    model,
    messages,
    maxTokens: (_b = (_a = input.config) == null ? void 0 : _a.maxOutputTokens) != null ? _b : 1024,
    temperature: (_d = (_c = input.config) == null ? void 0 : _c.temperature) != null ? _d : 0.7
  }, ((_e = input.config) == null ? void 0 : _e.topP) && { topP: input.config.topP }), ((_f = input.config) == null ? void 0 : _f.stopSequences) && { stop: input.config.stopSequences }), input.tools && {
...
function fromMistralResponse(_input, response) {
  var _a;
  const firstChoice = (_a = response.choices) == null ? void 0 : _a[0];
  const contentParts = (firstChoice == null ? void 0 : firstChoice.message) ? fromMistralMessage(firstChoice.message) : [];
  const message = {
    role: "model",
  ...
  return {
    message,
    finishReason: fromMistralFinishReason(firstChoice == null ? void 0 : firstChoice.finishReason),
    usage: {
...
function mistralModel(ai, modelName, projectId, region) {
  ...
  return ai.defineModel(
    ...
    (input, streamingCallback) => __async(this, null, function* () {
      ...
      const client = getClient(((_a2 = input.config) == null ? void 0 : _a2.location) || region);
      const versionedModel = (_f = (_e = (_b2 = input.config) == null ? void 0 : _b2.version) != null ? _e : (_d = (_c2 = model.info) == null ? void 0 : _c2.versions) == null ? void 0 : _d[0]) != null ? _f : model.name;
      if (!streamingCallback) {
        const mistralRequest = toMistralRequest(versionedModel, input);
        const response = yield client.chat.complete(mistralRequest, {
          fetchOptions: {
            headers: {
        ...
        const mistralRequest = toMistralRequest(versionedModel, input);
        const stream = yield client.chat.stream(mistralRequest, {
          fetchOptions: {
            headers: {
              "X-Goog-Api-Client": GENKIT_CLIENT_HEADER
            }
        ...
        const completeResponse = yield client.chat.complete(mistralRequest, {
          fetchOptions: {
            headers: {
              "X-Goog-Api-Client": GENKIT_CLIENT_HEADER
            }
...
function fromMistralCompletionChunk(chunk) {
  var _a, _b;
  if (!((_b = (_a = chunk.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.delta)) return [];
